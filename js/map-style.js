...
function overlayLayers(theme) {
  ...
  return [
    ...
    // Стрелки направления движения вдоль линии маршрута.
    [{ id: 'route-arrows', type: 'symbol', source: 'route', minzoom: 13,
      layout: { 'symbol-placement': 'line', 'symbol-spacing': 70, 'text-field': '›', 'text-font': ['Noto Sans Bold'],
        'text-size': ['interpolate', ['linear'], ['zoom'], 13, 16, 18, 26], 'text-keep-upright': false,
        'text-rotation-alignment': 'map', 'text-allow-overlap': true, 'text-ignore-placement': true },
      paint: { 'text-color': C.routeCasing } }, 'street-name'],
    [{ id: 'dest-halo',
 type: 'circle', source: 'dest', paint: { 'circle-radius': 18, 'circle-color': C.route, 'circle-opacity': 0.25 } }],
